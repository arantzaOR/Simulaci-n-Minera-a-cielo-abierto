...
# REALMENTE NO ES Z=0 PYTHON NO RECONOCE Z NEGATIVOS EN ANIMACIÓN 3D
...
# esto se hace sólo para simplificar el código
...
PUNTOS_CHANCADOR = {
    ...
    "chancadora_2": {"x": (chanc_x0+chanc_x1)/2, "y": chanc_y0, "z": 10},
...
Aquí se modifica la configuración general de la simulación
(número de camiones y sus destinos).
...
recurso_pala = sim.Resource("Pala", capacity=3)
recurso_chancador = sim.Resource("Chancador", capacity=3)
''' Capacidad 3 para permitir que hasta 3 camiones carguen simultáneamente '''

# Sistema de destinos (se mantiene igual)
cola_destino = {
...
class Camion(sim.Component):
    ...
    def color_camion(self, t):
        ...
        if self.estado in ['yendo', 'colando_destino', 'en_cola_destino', 'descargando']:
            return 'mediumorchid'
        else:
    ...
    # Estoy orgullosa de esta función :)
    # fue una locura
    def asignar_punto_carga(self):
        '''Asigna punto de carga'''
    ...
    def asignar_punto_chancador(self):
        '''Asigna punto de chancador usando round-robin'''
    ...
    def calcular_posicion_cola_destino(self, destino):
        ...
        # Si es planta y tiene punto de chancador asignado, usar ese punto
        if self.current_destino == 'planta' and self.punto_chancador_asignado:
            punto = PUNTOS_CHANCADOR[self.punto_chancador_asignado]
            return punto['x'], punto['y'], z_max
    
        # Para otros destinos o si no tiene punto asignado
        if self.posicion_en_cola is None:
            return dest_x, dest_y+2, z_max
        
        distancia = (self.posicion_en_cola + 1) * espacio_camiones
    
        # Para planta sin punto asignado, organizar cola horizontalmente
    ...
    #         if self.current_destino == 'planta' and self.punto_chancador_asignado:
    #             punto = PUNTOS_CHANCADOR[self.punto_chancador_asignado]
    #             print(f"Camion {self.id} - Primer en cola planta - Punto: {self.punto_chancador_asignado}")
    #             return punto['x'], punto['y'], z_max
    #         else:
    #             return dest_x, dest_y, z_max
    #     distancia = (self.posicion_en_cola + 2) * espacio_camiones
    #     print(f"Camion {self.id} - En cola planta - Pos: {dest_x}, {dest_y+distancia}")
    #     return dest_x , dest_y + distancia, z_max
    ...
    # ====== Funciones de posición ACTUALIZADAS ====== #
    def x_truck(self, t):
        ...
        if self.estado == 'en_cola_punto_carga':
            ...
            if t_rel <= T_espiral:
                ...
                return dest_x
        elif self.estado in ['cola_destino', 'en_cola_destino', 'descargando']:
            # if self.current_destino == 'planta' and self.punto_chancador_asignado:
            #     punto = PUNTOS_CHANCADOR[self.punto_chancador_asignado]
            #     return punto["x"]
    ...
    def y_truck(self, t):
        ...
        if self.estado == 'en_cola_punto_carga':
            x, y, z = self.calcular_posicion_cola_puntos_carga()
            return y
        if self.estado == 'esperando_en_pala':
            punto = PUNTOS_CARGA[self.punto_carga_asignado]
            return punto["y"]
        elif self.estado == "yendo_punto_carga" and self.punto_carga_asignado:
            punto = PUNTOS_CARGA[self.punto_carga_asignado]
    ...
    def process(self):
        ...
        while True:
            ...
            self.estado = "yendo_punto_carga"
            self.start_time = env.now()
            self.hold(2.0)  # Tiempo para moverse al punto
            
            self.estado = "esperando_en_pala"
            self.start_time = env.now()
            ...
            if self.current_destino == 'planta':
                self.request(recurso_chancador)
            ...
            if self.current_destino == 'planta':
                self.release(recurso_chancador)
